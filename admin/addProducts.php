...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($colors_data as $i => &$colorEntry) {
        ...
        if (isset($_FILES[$fileKey]) && $_FILES[$fileKey]['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array($ext, ['jpg', 'jpeg', 'png', 'gif', 'webp'])) {
                $mime = (new finfo(FILEINFO_MIME_TYPE))->file($_FILES[$fileKey]['tmp_name']);
    ...
    if (empty($errors)) {
        try {
            ...
            $stmt = $pdo->prepare("INSERT INTO products (name, description, price, discount_price, category_id, subcategory, brand, stock, image_url, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
            $stmt->execute([$name, $description, $price, $dp, $category_id, $subcategory, $brand, $stock, $image_url]);
            ...
            if (!empty($category_id)) {
                ...
                $cat = $catStmt->fetch();
                if ($cat) $categorySlug = generateCategorySlug($cat['name']);
            }
            $slug = generateProductSlug($name, $newProductId, $categorySlug);
            $pdo->prepare("UPDATE products SET slug = ? WHERE id = ?")->execute([$slug, $newProductId]);
            ...
            foreach ($colors_data as $i => $colorEntry) {
                if (!empty($colorEntry['name'])) {
                    $variantStmt->execute([$newProductId, 'color', $colorEntry['name'], $colorEntry['image_url'] ?? null, $i]);
                }
            }
            if (!empty($sizes_raw)) {
                foreach (array_filter(array_map('trim', explode(',', $sizes_raw))) as $i => $s) {
                    $pdo->prepare("INSERT INTO product_variants (product_id, type, value, display_order) VALUES (?,?,?,?)")->execute([$newProductId, 'size', $s, $i]);
                }
            }
            $pdo->commit();
...
<head>
    ...
    <title>Add Product - SHOPWAVE Admin</title>
    ...
    <style>
        ...
        .url-preview-wrapper { margin-top: 1rem; text-align: center; }
        .url-preview-wrapper img {
            max-width: 200px; max-height: 200px;
            border-radius: 0.5rem; border: 2px solid var(--border);
            box-shadow: 0 4px 8px rgba(0,0,0,0.1);
        }
...
<body>
            ...
            <main class="admin-main">
                <header class="admin-header">
                    <div style="display:flex;align-items:center;gap:1rem;">
                        ...
                        <h1 style="font-size:1.1rem;margin:0;">Add New Product</h1>
                    </div>
                </header>
                        ...
                        <form method="POST" enctype="multipart/form-data" id="productForm">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="name" class="form-input" placeholder="Enter product name"
                                    value="<?php echo htmlspecialchars($_POST['name'] ?? ''); ?>" required maxlength="255">
                            </div>

                            <div class="form-group">
                                <label class="form-label">Description</label>
                                <textarea name="description" class="form-textarea" placeholder="Enter product description" maxlength="5000"><?php echo htmlspecialchars($_POST['description'] ?? ''); ?></textarea>
                            </div>
                            ...
                            <div class="form-grid">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="number" name="discount_price" class="form-input" placeholder="0.00" step="0.01" min="0"
                            ...
                            <div class="form-grid">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="brand" class="form-input" placeholder="Enter brand name" maxlength="100"
                            ...
                            <div class="form-grid">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="subcategory" class="form-input" placeholder="e.g., Tops, Hats" maxlength="100"
                                        value="<?php echo htmlspecialchars($_POST['subcategory'] ?? ''); ?>">
                                </div>
                            </div>

                            <div class="variant-section-title">Main Product Image</div>
                            <div class="form-help" style="margin-bottom:1rem;">This is the default image shown before any color is selected.</div>

                            <div class="form-group">
                                <label class="image-upload" for="image-input">
                                    <div style="font-size:2rem;margin-bottom:0.5rem;">📷</div>
                                    <div style="font-weight:600;">Click to upload main image</div>
                                ...
                                <div style="display:flex;align-items:center;gap:1rem;margin:var(--space-sm) 0;">
                                    <div style="flex:1;height:1px;background:var(--border);"></div>
                                    <span style="color:var(--muted);font-size:var(--fs-sm);font-weight:600;">OR</span>
                                    <div style="flex:1;height:1px;background:var(--border);"></div>
                                </div>
                                <input type="url" name="image_url_input" id="image-url-input" class="form-input"
                                    placeholder="https://example.com/image.jpg"
                                    value="<?php echo htmlspecialchars($_POST['image_url_input'] ?? ''); ?>">
                                <div class="url-preview-wrapper" id="urlPreview" style="display:none;">
                                    <img id="urlPreviewImg" src="" alt="URL Preview">
                                </div>
                            </div>

                            <div class="variant-section-title">Color Variants</div>
                            <div class="form-help" style="margin-bottom:1rem;">Each color can have its own product image. Clicking a color on the product page will switch to that image.</div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="hidden" name="sizes" id="sizesHidden" value="">
                                <div class="form-help">Sizes shown as selectable options on the product page</div>
                            </div>
    ...
    <script>
        window.INITIAL_COLORS = [];
        window.INITIAL_SIZES = [];
    </script>
